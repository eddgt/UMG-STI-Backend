...
class LoginController {
    ...
    public async loginAuth(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            const login = await conne.query('SELECT \'usuario válido\' message,  usuario, rol, id_rol, nombre, email FROM Usuarios WHERE email = ? and pass = ?', [newLogin.email, newLogin.pass]);
